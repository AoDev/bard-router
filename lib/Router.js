...
function copyRequest(request) {
  return {
    route: request.route,
    params: request.params ? (0, _extends2.default)({}, request.params) : {}
  };
}
/**
...
function traverse(router, pathNodes, currentNode, request) {
  const {
    routes
  } = router;
  ...
  if (routes[currentNode]) {
    ...
    if (typeof routes[currentNode].onTheWay === 'function') {
      ...
      if (typeof onTheWayResult.route === 'string' && onTheWayResult.route !== request.route) {
         ...
         */
        newPathNodes[newIndex] === pathNodes[newIndex] && newIndex < newPathNodes.length && newIndex < pathNodes.length && newIndex < segmentIndex) {
          newIndex++;
        }
...
class Router {
  ...
  goTo(request) {
    ...
    const currentRouteConfig = this.routes[currentState.route]; // for dev it's easier to not provide empty params when calling goTo(), so we provide default here
    ...
    if (!routeConfig) {
      logger.warn('404 - Unknown route'); // TODO route not found
    }
...
Router.copyRequest = copyRequest;
Router.splitPath = splitPath;